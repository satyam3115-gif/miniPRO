...
found:
  ...
  p->queue = 0;
  p->ticks_consumed = 0;
  p->seq = __atomic_add_fetch(&proc_seq, 1, __ATOMIC_SEQ_CST);
#endif
...
kexit(int status)
{
  ...
  release(&tickslock);
  if (p->pid > 2) {
    printk("PID %d: TAT=%d, WT=%d, RT=%d\n", p->pid, p->etime - p->ctime, p->wtime, p->rtime - p->ctime);
  }
...
scheduler(void)
{
  ...
  for (;;) {
    ...
    release(&tickslock);
    if (current_ticks - last_boost_tick >= 48) {
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if (p->state == RUNNABLE) {
        if (p->queue < best_queue || (p->queue == best_queue && p->seq < best_seq)) {
          best_queue = p->queue;
          best_seq = p->seq;
          best_p = p;
        }
    ...
    if (best_p) {
      acquire(&best_p->lock);
      if (best_p->state == RUNNABLE && best_p->queue == best_queue && best_p->seq == best_seq) {
        if (best_p->rtime == 0) {
          acquire(&tickslock);
          best_p->rtime = ticks;
          release(&tickslock);
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if (p->state == RUNNABLE) {
        if (best_p == 0 || p->ctime < best_ctime) {
          best_ctime = p->ctime;
          best_p = p;
        }
      }
    ...
    if (best_p) {
      acquire(&best_p->lock);
      if (best_p->state == RUNNABLE && best_p->ctime == best_ctime) {
        if (best_p->rtime == 0) {
          acquire(&tickslock);
          best_p->rtime = ticks;
          release(&tickslock);
  ...
  if (p->ticks_consumed >= slice) {
    ...
    for (other_p = proc; other_p < &proc[NPROC]; other_p++) {
      if (other_p != p) {
        ...
        if (other_p->state == RUNNABLE && other_p->queue < p->queue) {
          should_yield = 1;
        }
...
procdump(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
#ifdef MLFQ
    printk("%d %s %s [q%d consumed=%d seq=%lu]", p->pid, state, p->name, p->queue, p->ticks_consumed, p->seq);
#else
...
// Update process metrics. Called from clockintr().
void
update_process_metrics(void)
{
  struct proc *p;
  for (p = proc; p < &proc[NPROC]; p++) {
    acquire(&p->lock);
...
// Dump process queues to console for MLFQ testing
void
dump_queues(void)
{
#ifdef MLFQ
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    ...
    if (p->state == RUNNABLE || p->state == RUNNING) {
      printk("[TICK %d] PID %d: QUEUE %d\n", current_ticks, p->pid, p->queue);
    }
    release(&p->lock);
  }
#endif
